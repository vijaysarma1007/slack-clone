  ...
  handler: async (ctx, args) => {
    ...
    const dedupedReactions = reactionsWithCounts.reduce(
      (acc, reaction) => {
        ...
        if (existingReaction) {
          existingReaction.membersIds = Array.from(
            new Set([
              ...existingReaction.membersIds,
              reaction.memberId,
            ])
          );
        } else {
      ...
      page: await Promise.all(
        results.page
          .map(async (message) => {
            ...
            const reactionsWithoutMemberIdsProperty =
              dedupedReactions.map(({ memberId, ...rest }) => rest);
...
export const create = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const messagedId = await ctx.db.insert("messages", {
      memberId: member._id,
      body: args.body,
      image: args.image,
